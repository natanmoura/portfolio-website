...
// How far back the opening shot on load is allowed to sit.
//
// Framing fits the whole town, and on a big one "the whole town" is a long way
// off: a 1000x1530 sprawl with props floating 900 up fits at 2900 units, where
// a building is a few pixels wide, every collage face is sampling a coarse mip
// and the town reads as a soft smear. That is a fine wide shot and a poor
// place to start work.
//
// Until the zoom clamp was sized to the town it was also unreachable — the old
// flat 1500 quietly swallowed the difference, and what you actually got on
// load was this view. Which is the view to keep: near enough that the collage
// is legible, far enough to see most of a large town at once. F still fits
// everything, so the wide shot is one key away rather than the only option.
//
// A small town fits well inside this and is unaffected: its framing distance
// is the smaller of the two and stays exactly as it was.
const OPEN_DISTANCE = 1500;

// Framing needs real bounds, so finish any queued meshing first.
function frameCity(limit) {
  builder.flushAll();
  stage.frame(builder.root, limit);
}
