...
export const CONTROL_DEFS = [
  ...
  },
  {
    ...
    items: [
      ...
      R('roadEase', 'Slope easing', 0, 1, 0.01, 'How far a road will leave the ground to keep its own slope reasonable. Zero glues it to the terrain, over every bump and down every cliff. Raise it and the road refuses steeper and steeper ground: the descent off a hill stretches out, rounds into an S instead of a straight ramp, and leaves a taller gap underneath for the columns to fill.', { live: false, hard: [0, 1] }),
      R('roadColumnSpacing', 'Column spacing', 2, 30, 0.5, 'How far apart the supports under a raised road stand, measured along it. The road is divided into a whole number of equal bays, so they always come out evenly spaced end to end.', { live: false, hard: [1, 200] }),
      R('roadColumnWidth', 'Column thickness', 0.05, 1.5, 0.01, 'How thick each support is, as a diameter. One thickness for the whole town: a highway is held up by a pair, one under each edge of its deck, and a street by a single one down the middle of its path, so the count is what tells them apart rather than the girth. Thin on purpose -- the thing worth looking at is the road being up in the air, not what is holding it there.', { live: false, hard: [0.02, 20] }),
      {
        key: 'roadBridging',
      ...
      R('roadGlow', 'Road glow', 0, 3, 0.01, 'Light coming out of the tarmac itself, rather than off it. Because it is emissive it feeds the bloom pass, so a lit road blooms into the air above rather than just reading as a paler stripe. Columns take a third of it, so a viaduct keeps its deck as the source and its piers as structure. Unlike lit windows this does not follow the hour, so a road you light stays lit at noon.', { cheap: true, hard: [0, 12] }),
      {
        key: 'roadGlowColor',
        label: 'Road glow colour',
        type: 'color',
        cheap: true,
        help: 'What colour the tarmac gives off. Warm reads as street lighting, cold or saturated reads as a graphic line through the town.',
      },
  ...
  },
  {
    ...
    items: [
      ...
      R('terrainHeight', 'Hill height', 0, 20, 0.1, 'How far the rolled ground rises and falls. Buildings stay planted on a slope. Hills only -- drawn ground ignores this and everything else in this group.', { live: false, hard: [0, 300] }),
      R('terrainScale', 'Hill size', 0.1, 4, 0.01, 'How wide the bumps are. Large gives a few broad hills the town drapes over. Hills only.', { live: false, hard: [0.02, 40] }),
      R('terrainDetail', 'Roughness', 1, 5, 1, 'Layers of noise. One is smooth swells, five adds fine crumple on top. Hills only -- a drawn shape carries its own roughness, set by selecting it.', { live: false, hard: [1, 8] }),
      R('terrainStep', 'Terracing', 0, 8, 0.1, 'Cuts the rolled ground into flat shelves this far apart, with a hard riser between each. Zero is off. Hills only -- a drawn shape carries its own terracing, so one mesa can be stepped while another beside it stays smooth.', { live: false, hard: [0, 80] }),
      R('maxBuildSlope', 'Buildable slope', 5, 60, 1, 'The steepest ground anything will be built on, in degrees. Applies to both kinds of ground. Nothing stands on a cliff face past this: the plots are dropped and the ground is left bare, so a mesa gets a town on top and clean sides. Measured across a plot’s whole footprint, not at its centre, so one corner hanging over an edge is enough to reject it. Ninety turns it off.', { live: false, hard: [1, 90] }),
      {
        key: 'landformTools',
        label: 'Drawn ground',
        type: 'mount',
  ...
  },
  {
    ...
    items: [
      ...
      R('particleSpeedVariance', 'Speed spread', 0, 1, 0.01, 'How much they differ from each other in speed. Zero moves the whole field as one sheet, which reads as a scrolling texture rather than as objects. High gives stragglers and racers. The average stays exactly on the Speed above whichever way this goes.', { live: false, hard: [0, 1] }),
